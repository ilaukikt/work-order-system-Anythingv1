...
export async function PUT(request, { params }) {
  try {
    ...
    if (data.vendor_type !== undefined && !["Service Provider", "Contractor"].includes(data.vendor_type)) {
      return Response.json(
    ...
    if (data.default_retention_percent !== undefined && ![0, 5, 10].includes(Number(data.default_retention_percent))) {
      return Response.json(
    ...
    const updateableFields = [
      'vendor_name', 'vendor_type', 'contact_person', 'contact_number',
      'email', 'address', 'gst_number', 'pan_number', 'bank_name',
      'bank_account_number', 'bank_ifsc', 'default_retention_percent', 'status'
    ];
  ...
  try {
    ...
    if (workOrderCheck.length > 0) {
      return Response.json(
        {
          success: false,
          error: "Cannot delete vendor. This vendor is referenced in existing work orders.",
        },
        { status: 400 },
      );
    }
